...
Usage:
    python3 run_delta_fit.py --frames 50000000 --nmin 190 --nmax 1000 --count 10
"""
...
def find_primes(n_min=190, n_max=1000, count=10):
    ...
    return [all_p[round(i * step)] for i in range(count)]

def h_filename(p):
    n, k = p + 1, (p + 1) // 2
    for c in [f"H_{n}_{k}.bin", f"H_{n}.bin", f"H{n}_{k}.bin", f"H{n}.bin"]:
        if os.path.exists(c):
            return c
    return f"H_{n}_{k}.bin"
...
def run_sim(p, nframes, nthr):
    ...
    cmd = ["./eqr_bec_sim", "-b", fname, "-f", str(nframes),
    ...
    for line in ret.stdout.strip().split("\n"):
        line = line.strip()
        if not line or line.startswith("ep") or line.startswith("--") \
                or line.startswith("Code") or line.startswith("Done"):
            continue
...
def main():
    ...
    parser.add_argument("--nmin", type=int, default=190)
    ...
    if not os.path.exists("./eqr_bec_sim"):
        print("ERROR: ./eqr_bec_sim not found. Compile first:")
        print("  gcc -O3 -march=native -fopenmp -o eqr_bec_sim eqr_bec_sim.c -lm")
        return 1
    ...
    for p in primes:
        ...
        if pamb is not None and pamb == 0.0:
            print(f"  >>> P_amb = 0 at n={n}, skipping remaining")
            stopped_early = True
