...
static HMat *build_eqr_hmat(int p) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        int row = 0;
        for (int j = 0; j < deg; j++) if (rem[j]) row |= (1 << j);
        row |= (1 << (i + deg));
        Grows[i] = row;
    }
    free(rem); free(g);
 ...
 *  Simulation
 * ================================================================ */
static double simulate_eps_bitmmap(const HMat *H, double eps, int nframes, int nthr) {
    long long tot_amb = 0;
...
static double simulate_eps_blockmap(const HMat *H, double eps, int nframes, int nthr) {
    long long tot_fail = 0;

    #pragma omp parallel num_threads(nthr) reduction(+:tot_fail)
    {
        int tid = omp_get_thread_num();
        rng_t rng;
        rng_seed(&rng, 42ULL + (uint64_t)tid * 1000003ULL +
        ...
        for (int f = 0; f < nframes; f++) {
            ...
            /* Bit 0 always erased */
            ge_insert(ws, hcol(H, 0));
            n_erased++;

            /* Bits 1..n-1 erased i.i.d. with prob eps */
            for (int j = 1; j < H->n; j++) {
                if (rng_double(&rng) < eps) {
                    ge_insert(ws, hcol(H, j));
                    n_erased++;
            ...
            /* Block-MAP fails if erased columns are NOT full rank */
            if (ws->rank < n_erased)
                tot_fail++;
        }
        ge_free(ws);
    }
    return (double)tot_fail / nframes;
}
...
int main(int argc, char **argv) {
    ...
    char  *csvfile = NULL;
    int    nframes = 0;
    double eps_s = 0.38, eps_e = 0.62, eps_d = 0.02;
    ...
    if (csvfile) {
        csv = fopen(csvfile, "w");
        if (csv) fprintf(csv, "epsilon,P_block_fail,P_bit0_ambig,frames,time_sec\n");
    }

    printf("%-12s %-18s %-18s %-10s %s\n",
           "epsilon", "P_block_fail", "P_bit0_ambig", "frames", "time");
    printf("%-12s %-18s %-18s %-10s %s\n",
           "--------", "----------", "----------", "------", "----");
    ...
    for (double eps = eps_s; eps <= eps_e + eps_d * 0.01; eps += eps_d) {
        ...
        double p_block = simulate_eps_blockmap(H, eps, nframes, nthr);
        double p_bit   = simulate_eps_bitmmap(H, eps, nframes, nthr);
        double dt = omp_get_wtime() - t0;

        printf("%-12.4f %-18.8f %-18.8f %-10d %.3fs\n",
               eps, p_block, p_bit, nframes, dt);
        fflush(stdout);

        if (csv) {
            fprintf(csv, "%.6f,%.10f,%.10f,%d,%.4f\n",
                    eps, p_block, p_bit, nframes, dt);
            fflush(csv);
        }
